function L = gfilterCustom( L, sigma, orders, boundaryOption )
...
BOUNDARY_OPTION = 'replicate';


%% CHECK INPUT

% check amount of arguments
if nargin>4;  error([mfilename ': too many parameters!']);  end;
...
if(nargin>3)
    BOUNDARY_OPTION = boundaryOption;
else
...
for d = 1 : dims
    ...
    k = gaussiankernelCustom(sigma, orders(d));
    %size(k)
    %k
    ...
    % convolve
    L = imfilter( L, k, BOUNDARY_OPTION, 'same','conv' );
    %mean2(L)
end
